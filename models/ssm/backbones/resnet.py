...
class ResNet(nn.Module):
    ...
    def __init__(self, arch, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        # Alex: in ResNet18 this output the last module in the penultimate block
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    # add architecture name to the model
    model = ResNet(arch, block, layers, **kwargs)
    if pretrained:
       print("Loading pretrained weights")
       model.load_state_dict(load_state_dict_from_url(model_urls[arch]))
    return model
